...
- **Session history.** Finished sessions stay listed; anything still waiting on an answer is kept regardless of age, so you can leave and come back. Finished rows fade out after five minutes (configurable), or you can dismiss them one at a time or all at once — but never one that still has a question on it. Codex turn notifications are grouped into one row per chat, even when a notification omits its conversation ID.
- **Background tasks are labelled.** When an agent reports that it is running in the background, its session row carries a distinct background badge.
