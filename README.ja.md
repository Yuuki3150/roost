...
公開向けの紹介は [README.md](README.md)。こちらは設計判断と、なぜそうしたかの記録。
...
ローカルサーバーは起動時に `47821`〜`47830` の空いているポートにバインドし、実際に使ったポート番号を `%LOCALAPPDATA%\Roost\port.txt` に書き出す。エージェント連携スクリプトはこのファイルを読んでポートを特定する。
...
`status` が `done` / `closed` になっても一覧からは消えず、履歴として残る（[完了したセッションを消す](#完了したセッションを消す)を参照）。
...
ノッチのフッターのトグルで切り替える。状態は `%LOCALAPPDATA%\Roost\approval-mode.json` に保存され、hookスクリプトが同期的に読む。
...
**ジャンプ先PIDの解決方法。** hookスクリプトが見えるのは自分の親プロセス（＝エージェントが起動した**ウィンドウを持たないシェル**）だけなので、そのPIDをそのまま使うとジャンプは無反応になる。プロセスツリーを遡って**実際に可視のトップレベルウィンドウを所有している**最初の祖先を探す方式にした。この判定はWin32が要るのでRust側（`window_in_tree`）で行い、hookは `process.ppid` を送るだけにしている。

> 以前はスクリプト側でプロセス**名**を照合していた（`powershell.exe` なら端末とみなす等）が、これは誤りだった。hook自身のシェルが `powershell.exe` なので必ずそこで止まってしまい、ウィンドウを持たないPIDが記録されていた。名前ではなく**ウィンドウの有無**で判定すること。この方式なら、CLIをGUIアプリが抱えている場合（Claudeデスクトップアプリ、Cursor等）も名前を列挙せずに正しく解決できる。

解決はイベント受信時に行い、結果をセッションに保存する。ターン終了後にはhookのプロセスツリーが消えているため、クリック時に遡っても手遅れになる。

`SetForegroundWindow` は、呼び出し元が現在の前面ウィンドウを持っていないと Windows に拒否される。行クリックの場合はオーバーレイが前面なので通るが、グローバルホットキー経由では通らない。そのため失敗時は `AttachThreadInput` で一時的に前面スレッドの入力キューを共有してリトライする。
...
履歴が溜まると邪魔になるので、消す手段が3つある。

| 方法 | 動作 |
|---|---|
| 自動 | 完了から既定5分で消える。設定の「完了を隠すまで」で0〜60分、0で無効 |
| 行の ✕ | 完了行にホバーすると出る。その行だけ消す |
| 「完了を消す」 | フッターのボタン。消せる完了行がある時だけ出る |

**自動と一括は「まだ回答が必要なもの」を消さない。** 質問が残っているセッションは `needs_attention()` に該当するので対象外で、この判定は `state.rs` のテストで固定してある。一方、**行ごとの ✕ は質問が残っていても消す** — ユーザーが明示的に「これはもういい」と言っているため。自動的に情報を失わせないという設計判断であって、手動操作を縛るものではない。

自動で隠す判定はフロントエンド側で行う（`isExpired`）。パネルは経過時間の表示用にもともと30秒ごとに再描画しているので、それに乗るだけで済む。バックエンドには残るため、同じセッションIDで再開すれば行も戻る。

一方 **✕ と一括クリアはバックエンドから消す**（`dismiss_session` / `clear_finished`）。次のブロードキャストは一覧をバックエンドから作り直すので、UI側だけで消しても即座に復活してしまう。

## 設定

フッターの ⚙ ボタンから開く。`%LOCALAPPDATA%\Roost\settings.json` に保存され、変更は即座にウィンドウへ反映される。
...
| 完了を隠すまで | 完了した行を何分後に自動で消すか（既定5分、0で消さない）。回答待ちのものは対象外 |
